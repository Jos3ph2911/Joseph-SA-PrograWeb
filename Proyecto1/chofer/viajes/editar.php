...
$fecha_hora = date('Y-m-d\TH:i', strtotime($viaje['fecha_hora']));
$costo_por_espacio = $viaje['costo_por_espacio'];
$espacios_totales = $viaje['espacios_totales'];
$vehiculo_id = $viaje['id_vehiculo'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errores)) {
        $sql = "UPDATE viajes 
                SET id_vehiculo = ?, titulo = ?, lugar_salida = ?, lugar_llegada = ?, 
                    fecha_hora = ?, costo_por_espacio = ?, espacios_totales = ?, espacios_disponibles = ?
                WHERE id = ? AND id_chofer = ?";
        ...
        if ($stmt->execute()) {
            $exito = "✅ Viaje actualizado correctamente.";
        } else {
            $errores['general'] = "❌ Error al actualizar el viaje. Intente nuevamente.";
        }
    }
}
?>
